...
import { getDataWithComp, getDocStatusColour, getAlert, getMaxPayNo } from '../../../utils/SamuiUtils';
...
function PaymentVoucher() {
  ...
  const fetchRealtime = async () => {
    try {
      const masterList = await getDataWithComp('API_0401_PAY_H', 'ORDER BY Pay_No DESC');
      // const detailList = await getDataWithComp('API_0402_PAY_D', '');
      ...
      const findMaxPayNo = await getDataWithComp('PAY_H', 'ORDER BY Pay_No DESC');
