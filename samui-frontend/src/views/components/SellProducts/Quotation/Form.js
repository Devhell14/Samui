...
// Model
import { ArMasterModel } from '../../../../model/SellProducts/ArMasterModel';
import { ArDetailModel } from '../../../../model/SellProducts/ArDetailModel';
...
function Form({ callInitialize, mode, name, maxDocNo }) {
  const [formMasterList, setFormMasterList] = useState(ArMasterModel());
  ...
  // console.debug('formMasterList =>', formMasterList)
  // การคำนวณเงิน
  ...
  const getModelByNo = async (arDataList) => {
    // console.debug('getModelByNo arDataList =>', arDataList)
    try {
      // ดึงข้อมูลเอกสารจาก AR_H
      const [findMaster] = await Promise.all([
        getDataWithComp('QT_H', ''),
      ]);
      const fromDatabase = findMaster.find(ar => ar.Doc_No === maxDocNo);

      if (!fromDatabase) {
        throw new Error("ไม่พบข้อมูลเอกสาร");
      }

      // // ดึงข้อมูลลูกค้าจาก arDataList โดยใช้ AR_ID
      // const fromViewAr = arDataList.find(ar => ar.AR_Id === fromDatabase.AR_ID);
      ...
      // // console.debug('findViewMaster =>', findViewMaster)
      ...
      const fromDetail = await getByDocId('QT_D', fromDatabase.Doc_Id, `ORDER BY Line ASC`);
      // console.debug('fromDetail =>', fromDetail)
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!formMasterList.arID && !formMasterList.arCode) {
        ...
        return; // หยุดการทำงานของฟังก์ชันหากไม่มีค่า apID หรือ arCode
      }
      ...
      // For Log PR_H
      // console.log("api/create-qt-h formMasterData =>", formMasterData);

      // ส่งข้อมูลหลักไปยัง API
      const response = await Axios.post(`${process.env.REACT_APP_API_URL}/api/create-qt-h`, formMasterData, {
      ...
      if (response.data.status === 'OK') {
        ...
        if (getDocIdResponse && getDocIdResponse.data.length > 0) {
          ...
          const detailPromises = formDetailList.map(async (item) => {
            ...
            return Axios.post(`${process.env.REACT_APP_API_URL}/api/create-qt-d`, formDetailData, {
              headers: { key: process.env.REACT_APP_ANALYTICS_KEY }
            });
          });
  ...
  const handleUpdate = async () => {
    try {
      ...
      // ส่งข้อมูลหลักไปยัง API
      const response = await Axios.post(`${process.env.REACT_APP_API_URL}/api/update-qt-h`, formMasterData, {
        headers: { key: process.env.REACT_APP_ANALYTICS_KEY }
      });
      ...
      if (response.data.status === 'OK') {
        ...
        await deleteDetail('  ', `WHERE Doc_ID = ${formMasterList.docId}`);
  ...
  const handleCancel = async () => {
    try {
      ...
      // ส่งข้อมูลหลักไปยัง API
      const response = await Axios.post(`${process.env.REACT_APP_API_URL}/api/cancel-qt-h`, formMasterData, {
        headers: { key: process.env.REACT_APP_ANALYTICS_KEY }
      });
  ...
  const onRowSelectAr = (arSelected) => {
    console.debug('arSelected =>', arSelected);
    try {
      setFormMasterList({
        ...formMasterList,
  ...
  const onRowSelectItem = (itemSelected) => {
    try {
      const newRow = ArDetailModel(formDetailList.length + 1);
        ...
        <div className="col-3 text-right">
          <div className="d-flex align-items-center">
            ...
            <input
              ...
              className="form-control input-spacing"
              name="approvedDate"
              value={formMasterList.arprovedDate || ''}
              onChange={handleChangeMaster}
              disabled={true} />
          </div>
        </div>
