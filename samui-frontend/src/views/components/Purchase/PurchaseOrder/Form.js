...
function Form({ callInitialize, mode, name, maxDocNo }) {
    ...
    // สำหรับเก็บตัวแปรเพื่อตรวจสอบว่า PO ตัวนี้มีใบรับหรือไม่ ?
    const [docIsRec, setDocIsRec] = useState(false);
    ...
    const getModelByNo = async (apDataList) => {
        try {
            ...
            // สำหรับเก็บตัวแปรเพื่อตรวจสอบว่า PO ตัวนี้มีใบรับหรือไม่ ?
            const findRecMasterByDocNo = await getDataWithComp('REC_H', `AND Ref_Doc = '${fromDatabase.Doc_No}' AND Rec_Status <> 13`);
            if (findRecMasterByDocNo && findRecMasterByDocNo.length > 0) {
                setDocIsRec(true);
            }
    ...
    const handleSubmit = async () => {
        try {
            ...
            const findMaxDocNo = await getDataWithComp('PO_H', 'ORDER BY Doc_No DESC');
    ...
    const handleCancel = async () => {
        try {
            // อัพสถานะรับสินค้าที่ PR_H (ยกเลิก PO แล้ว ใบ PR ตัวนั้นต้องกลับมาใช้ใหม่ได้)
            await updateStatusByNo(
                'PR_H',                                             // table: ชื่อตาราง
                'Doc_Is_PO',                                        // field: ชื่อฟิลด์
                0,                                                  // status: สถานะที่ต้องการอัพเดท
                `WHERE Doc_No = '${formMasterList.refDoc}'`         // where: เงื่อนไขในการอัพเดท
            );
    ...
    const handleRecover = async () => {
        try {
            ...
            const findRecMaster = await getDataWithComp("REC_H", `AND Ref_Doc = '${maxDocNo}' AND Rec_Status <> 13`);

            let docStatusReceive = parseInt("3", 10);
            ...
            const detailPromises = formDetailList.map(async (item) => {
                ...
                if (findRecMaster.length === 0 || (item.itemQty === 0 && item.itemRecQty === 0 && item.itemRecBalance === 0)) {
                    //อัพเดทจำนวนที่คำนวณใหม่ (โดยการ Where ด้วย DT_Id)
                    await updateQty(
                        'PO_D',
                        `Item_Qty = ${prDetail.Item_Qty}, 
                         Item_REC_Qty = ${parseInt("0.00", 10)}, 
                ...
                } else if (item.itemQty !== prDetail.Item_Qty) {
                    // => ให้ปรับจำนวนสินค้าตาม PR แล้วคำนวณ Balance ใหม่
                    let newItemQty = parseInt(prDetail.Item_Qty);
                    let newItemRecBalance = newItemQty - parseInt(item.itemRecQty);
                    ...
                    await updateQty(
                        ...
                        `Item_Qty = ${newItemQty}, 
                         Item_Total = ${prDetail.Item_Total}, 
                         Item_REC_Balance = ${newItemRecBalance}`,
                        `WHERE DT_Id = ${item.dtId} AND Item_Status = 1`
                    );

                    docStatusReceive = parseInt("2", 10);
                }
            });
            ...
            const formMasterData = {
                doc_no: formMasterList.docNo,
                doc_status: parseInt("2", 10),
                doc_status_receive: parseInt(docStatusReceive, 10)
            };
    ...
    const updateStatusClosePo = async () => {
        try {
            ...
            if (formMasterList.docStatusReceive === 1) {

                const findRecMasterByDocNo = await getDataWithComp('REC_H', `AND Ref_Doc = '${maxDocNo}' AND Rec_Status <> 13`);

                // กรณีที่ไม่เจอใบรับสินค้า หรือ ใบรับสินค้าถูกยกเลิกไปแล้ว
                if (findRecMasterByDocNo && findRecMasterByDocNo.length > 0) {
                    const detailPromises = formDetailList.map(async (item) => {
                        ...
                        if (Number(item.itemRecBalance) !== 0) {
                            ...
                            let itemRecQty = parseInt(item.itemQty);
                            ...
                            await updateQty(
                                'PO_D',
                                `Item_Qty = ${itemQty}, Item_REC_Qty = ${itemRecQty}, Item_REC_Balance = ${itemRecBalance}, Item_Total = ${updateItemTotal}`,
                    ...
                    const detailPromises = formDetailList.map(async (item) => {
                        // เงื่อนไขหาสินค้าที่รับไม่ครบ = จำนวนคงเหลือต้องไม่ = 0
                        // *************************************** UPDATE PO_D ****************************************
                        let itemQty = parseFloat("0.00");
                        let itemRecQty = parseFloat("0.00");
                        let itemRecBalance = parseFloat("0.00");
                        let itemDiscount = parseInt(item.itemDiscount);
                        let updateItemTotal = itemRecQty * parseCurrency(item.itemPriceUnit);
            ...
            } else if (formMasterList.docStatusReceive === 2) {
                // Loop Item
                const detailPromises = formDetailList.map(async (item) => {
                    // เงื่อนไขหาสินค้าที่รับไม่ครบ = จำนวนคงเหลือต้องไม่ = 0
                    if (Number(item.itemRecBalance) !== 0) {
                        ...
                        await updateQty(
                            'PO_D',
                            `Item_Qty = ${itemRecQty}, Item_REC_Balance = ${itemRecBalance}, Item_Total = ${updateItemTotal}`,
                            `WHERE Doc_ID = ${formMasterList.docId} AND Item_Id = ${item.itemId} AND Item_Status = 1`
                        );
    ...
    const onRowSelectPr = async (prSelected) => {
        try {
            ...
            const [getAllPrH, fromViewAp] = await Promise.all([
                getDataWithComp("API_0101_PR_H", "ORDER BY Doc_No DESC"),
            ...
            const getAllItem = await getDataWithComp('API_0102_PR_D', 'ORDER BY Line ASC');
