...
const queryDatabase = async (query, params, res) => {
    try {
        ...
        res.send({ "status": "OK", "message": "สร้างรายละเอียดรายการใบรับสินค้าสำเร็จ", "result": result });
    } catch (err) {
        console.log(err);
        res.send({ "status": "FAILED", "message": "สร้างรายละเอียดรายการใบรับสินค้าไม่สำเร็จ", "result": err });
    }
};

router.post('/create-rec-d', (req, res) => {
