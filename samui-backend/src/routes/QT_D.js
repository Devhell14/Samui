...
const queryDatabase = async (query, params, res, action) => {
    try {
        ...
        res.send({ "status": "OK", "message": `${action}รายละเอียดรายการใบเสนอราคาสำเร็จ`, "result": result });
    } catch (err) {
        console.log(err);
        res.send({ "status": "FAILED", "message": `${action}รายละเอียดรายการใบเสนอราคาไม่สำเร็จ`, "result": err });
    }
};

router.post('/create-qt-d', (req, res) => {
