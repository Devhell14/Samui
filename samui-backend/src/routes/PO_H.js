...
router.post('/create-po-h', (req, res) => {
    ...
    const apID = req.body.ap_id;
    const apCode = req.body.ap_code;
    ...
    const query = `
    INSERT INTO PO_H (
        Doc_No, Doc_Date, Doc_DueDate, Doc_Status, Doc_Status_Paid, 
        Doc_Status_Receive, Doc_Code, Doc_Type, Doc_For, Doc_Is_PRC, 
        Ref_DocID, Ref_Doc, Ref_DocDate, Ref_ProjectID, Ref_ProjectNo, 
        Comp_Id, Transport_Type, Doc_Remark1, Doc_Remark2, AP_ID, 
        AP_Code, Action_Hold, Discount_Value, Discount_Value_Type, 
        ...
        @docNo, @docDate, @docDueDate, @docStatus, @docStatusPaid, 
        @docStatusReceive, @docCode, @docType, @docFor, @docIsPRC, 
        @refDocID, @refDoc, @refDocDate, @refProjectID, @refProjectNo, 
        @compId, @transportType, @docRemark1, @docRemark2, @apID, 
        @apCode, @actionHold, @discountValue, @discountValueType, 
        ...
        @updateById, @approvedDate, @approvedByName, 
        @approvedById, @cancelDate, @cancelByName, 
        @cancelById, @approvedMemo, @printedStatus, 
        @printedDate, @printedBy
    )`;
    ...
    const params = [
        ...
        { name: 'apID', value: apID },
        { name: 'apCode', value: apCode },
...
router.post('/update-po-h', (req, res) => {
    if (!checkApiKey(req, res)) return;
    ...
    UPDATE PO_H SET
        ...
        Doc_Status_Paid = @docStatusPaid, Doc_Status_Receive = @docStatusReceive,
        Doc_Code = @docCode, Doc_Type = @docType, Doc_For = @docFor, 
        Doc_Is_PRC = @docIsPRC, Ref_DocID = @refDocID, Ref_Doc = @refDoc, 
...
router.post('/cancel-po-h', (req, res) => {
    if (!checkApiKey(req, res)) return;
...
router.post('/recover-po-h', (req, res) => {
    if (!checkApiKey(req, res)) return;
    ...
    const query = `UPDATE PO_H SET Doc_Status = @docStatus, Doc_Status_Receive = @docStatusReceive WHERE Doc_No = @docNo`;

    queryDatabase(query, params, res, "กู้สถานะคืน");
});
